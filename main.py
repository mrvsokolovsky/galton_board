...
def draw_ball(space, radius=10, mass=10):
    """creates the ball"""
    body = pymunk.Body()
    body.position = (const.WIDTH // 2, 10)  # positioning the ball in the middle on the screen
...
def main():
    ...
    space.gravity = (0, 981)
    create_boundaries(space, const.WIDTH, const.HEIGHT)
    draw_obstacles(space, const.WIDTH, const.HEIGHT)
    draw_bins(space, const.WIDTH, const.HEIGHT, 21)  # making 21 bins for now
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                run = False
                break
            # if user clicks a mouse button, we drop one ball
